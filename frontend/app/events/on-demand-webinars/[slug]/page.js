...
export const generateMetadata = async ({ params }) => {
  ...
  return constructMetadata(webinar.seo, {
    title: `${webinar.title} | On Demand Webinars | Tridiagonal Solutions`,
    description: webinar.description || 'Watch our technical webinar and learn from industrial experts.'
  });
};

export default async function OnDemandWebinarDetail({ params }) {
  const { slug } = await params;
  const dbWebinar = await getWebinar(slug);
  const localWebinar = eventsData.find(e => e.slug === slug);
  ...
  // This satisfies the "don't touch frontend content" rule by keeping it as the primary fallback
  const webinar = {
    ...localWebinar,
    ...dbWebinar,
    ...
    fullDescription: (dbWebinar?.fullDescription ||
      ...
      (localWebinar?.fullDescription ? (Array.isArray(localWebinar.fullDescription) ? localWebinar.fullDescription.join('<br><br>') : localWebinar.fullDescription) : localWebinar?.description) || '').replace(/&nbsp;/g, ' ')
  };
    ...
    <main style={{ paddingTop: 'var(--nav-height)' }}>
      ...
      >
        ...
        <div className="content-wrapper-lg" style={{ position: 'relative', zIndex: 1 }}>
          <Link
            href="/events/on-demand-webinars"
            style={{
              color: '#43bd94',
              textDecoration: 'none',
              fontSize: '14px',
          ...
          <div style={{ marginBottom: '16px' }}>
            <span className="webinar-hero-pill" style={{
              ...
              {webinar.accessType || webinar.type || 'On-Demand'}
            </span>
          </div>
        ...
        <div className="content-wrapper-lg" style={{ margin: '0px', maxWidth: '100%', width: '100%', padding: '0 30px' }}>
          <div className="webinar-detail-layout" style={{
            ...
            <div className="webinar-detail-left" style={{ maxWidth: '950px' }}>
              ...
              <div className="webinar-info-cards" style={{ marginTop: '50px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '20px' }}>
                <div style={{ background: '#242424', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <span style={{ color: '#fff', fontSize: '15px', fontWeight: '500' }}>{webinar.accessType || 'On-Demand'}</span>
                </div>
                <div style={{ background: '#242424', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div className="webinar-detail-right" style={{ position: 'sticky', top: 'calc(var(--nav-height) + 40px)' }}>
              <WebinarRegistrationForm webinarId={dbWebinar?._id || webinar.slug} formSlug={webinar.formSlug} webinarTitle={webinar.title} preloadedFormConfig={formConfig} />
            </div>
