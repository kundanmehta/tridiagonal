...
const FALLBACK_JOBS = [
  ...
  { id: 'sr-project-engineer-cfd-combustion', title: 'Sr. Project Engineer (CFD – Combustion Modeling)', department: 'Advanced Modeling & Simulation (CFD/FEA)', date: 'November 18, 2025', location: 'Pune, India', type: 'Full-time' },
  { id: 'project-engineer-efd-laboratory', title: 'Project Engineer (EFD Laboratory)', department: 'Technology Validation & Scale-up Centre', date: 'October 20, 2025', location: 'Pune, India', type: 'Full-time' },
  { id: 'project-leader-cfd-combustion', title: 'Project Leader (CFD - Combustion Modeling)', department: 'Advanced Modeling & Simulation (CFD/FEA)', date: 'February 13, 2025', location: 'Pune, India', type: 'Full-time' },
...
export default function Careers() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/careers/page`)
        ...
        if (d && Object.keys(d).length > 0) {
          setPageData({
            ...
            coreValuesSection: {
              ...
              description: d.coreValuesSection?.description || FALLBACK_PAGE.coreValuesSection.description,
              values: d.coreValuesSection?.values?.length > 0 ? d.coreValuesSection.values : FALLBACK_PAGE.coreValuesSection.values,
            },
            opportunitiesSection: {
              heading: d.opportunitiesSection?.heading || FALLBACK_PAGE.opportunitiesSection.heading,
            },
          });
        } else {
      ...
      .catch(() => setPageData(FALLBACK_PAGE));
  ...
  const p = pageData || FALLBACK_PAGE;

  /* ── Dynamic department list from live jobs ── */
    ...
    <main style={{ paddingTop: 'var(--nav-height)' }}>
      ...
      <section
        ...
        style={{
          position: 'relative',
          overflow: 'hidden',
          background: `url(${p.heroSection.bgImage}) center center / cover no-repeat`,
          minHeight: 'auto',
          padding: '80px 0 60px',
        }}
      >
        ...
        <div className="content-wrapper-lg" style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <h1
            ...
            style={{ color: '#fff', fontWeight: '700', fontSize: 'clamp(2.5rem, 5vw, 4rem)', marginBottom: '15px' }}
            dangerouslySetInnerHTML={{ __html: p.heroSection.title }}
          />
          <p className="hero-desc fade-in-up delay-200" style={{ color: 'rgba(255,255,255,0.9)', fontSize: '18px', maxWidth: '700px', margin: '0 auto' }}>
            {p.heroSection.description}
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 0', background: '#222' }}>
        <div className="content-wrapper-lg">
          <div style={{ marginBottom: '50px' }}>
            <h2 style={{ fontSize: 'clamp(2rem, 4vw, 2.8rem)', fontWeight: 'bold', color: 'var(--color-teal)', marginBottom: '12px' }}>
              {p.coreValuesSection.heading}
            </h2>
            <p style={{ fontSize: '18px', color: 'rgba(255,255,255,0.9)' }}>
              {p.coreValuesSection.description}
            </p>
          </div>
          <div className="core-values-grid">
            {p.coreValuesSection.values.map((val, idx) => (
              <div key={idx} className="core-value-card">
                ...
                <h3 style={{ fontSize: '20px', color: '#fff', fontWeight: '600', marginBottom: '12px' }}>
                  {val.title}
                </h3>
                <p style={{ color: 'rgba(255,255,255,0.95)', lineHeight: '1.7', fontSize: '18px' }}>
                  {val.desc}
                </p>
              </div>
            ))}
      ...
      <section className="section-pad" style={{ background: '#1a1a1a' }}>
        <div className="content-wrapper-lg">
          ...
          <div className="jobs-list">
            {loadingJobs && (
              <div style={{ textAlign: 'center', padding: '60px 20px', color: 'rgba(255,255,255,0.4)', fontSize: '16px' }}>Loading positions...</div>
            )}
            {!loadingJobs && filteredJobs.length === 0 && (
              <div style={{ textAlign: 'center', padding: '60px 20px', color: 'rgba(255,255,255,0.4)', fontSize: '16px' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .jobs-search-input { width: 100%; padding: 14px 15px 14px 44px; background: #242424; border: 1px solid rgba(255,255,255,0.1); border-radius: 10px; color: #fff; font-size: 15px; outline: none; transition: border-color 0.3s ease; }
        ...
        .job-apply-btn { padding: 10px 28px !important; font-size: 13px !important; font-weight: 700 !important; letter-spacing: 1px; white-space: nowrap; text-decoration: none; }
      `}} />
    </main>
  );
}
