...
export default function BrochuresClient() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/resources?type=Brochure`)
      .then(res => res.json())
      .then(json => {
        if (json.data) setBrochures(json.data);
        setLoading(false);
      })
    ...
    fetch(`${API_URL}/api/categories?type=Brochure`)
      ...
      .catch(() => setCategories(['All', 'Product', 'Solution', 'Corporate']));
  }, []);

  const filteredBrochures = brochures.filter(b => {
    const matchesSearch = (b.title || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
      (extractExcerpt(b.content) || '').toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#111', paddingTop: 'var(--nav-height)' }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg, #111 0%, #1a1a1a 100%)', padding: '100px 0 60px' }}>
        ...
        <div className="content-wrapper-lg" style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <h1 style={{ color: '#fff', fontWeight: '800', fontSize: 'clamp(32px, 6vw, 64px)', marginBottom: '20px', letterSpacing: '-0.03em' }}>
            Resource <span style={{ color: '#9b51e0' }}>Brochures</span>
          </h1>
          <p style={{ color: 'rgba(255, 255, 255, 0.5)', maxWidth: '700px', margin: '0 auto', fontSize: '1.1rem', lineHeight: '1.6' }}>
            Download our detailed brochures to learn more about our innovative solutions, products, and engineering services.
          </p>
        </div>
      </section>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', alignItems: 'center' }}>
              ...
              <div style={{ flex: '1 1 300px', position: 'relative' }}>
                ...
                <input
                  ...
                  onFocus={(e) => { e.target.style.borderColor = '#9b51e0'; }}
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '25px', flexWrap: 'wrap' }}>
              {categories.map(cat => (
                <button
                  ...
                  style={{
                    background: activeCategory === cat ? '#9b51e0' : 'rgba(255,255,255,0.03)',
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: '20px' }}>
              {filteredBrochures.map((b, i) => (
                <Link key={i} href={`/resources/brochures/${b.slug}`} style={{ textDecoration: 'none' }}>
                  <article className="brochure-card" style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#1a1a1a', borderRadius: '24px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)', transition: 'all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1)' }}>
                    ...
                    <div style={{ padding: '30px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                        ...
                        <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '11px' }}>
                          {b.date ? new Date(b.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : ''}
                        </span>
                      </div>
                      <h3 style={{ color: '#fff', fontSize: '18px', fontWeight: '600', lineHeight: '1.4', marginBottom: '15px' }}>{b.title}</h3>
                      <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '15px', lineHeight: '1.6', marginBottom: '25px', flex: 1 }}>{extractExcerpt(b.content, 130)}</p>
            ...
            <div style={{ textAlign: 'center', padding: '100px 0', background: 'rgba(255,255,255,0.01)', borderRadius: '32px', border: '2px dashed rgba(255,255,255,0.05)' }}>
              ...
              <button onClick={() => { setActiveCategory('All'); setActiveIndustry('All Industries'); setActiveService('All Services'); setSearchQuery(''); }} style={{ marginTop: '30px', background: 'transparent', border: '1px solid #9b51e0', padding: '12px 30px', borderRadius: '30px', fontWeight: '700', cursor: 'pointer', fontSize: '14px', transition: 'all 0.3s' }} onMouseEnter={(e) => { e.target.style.background = 'rgba(155, 81, 224, 0.05)'; }} onMouseLeave={(e) => { e.target.style.background = 'transparent'; }}>
                <span style={{ color: '#9b51e0' }}>Reset All Filters</span>
              </button>
            </div>
          )}
      ...
      <style jsx global>{`
        .brochure-card:hover { transform: translateY(-10px); border-color: rgba(155, 81, 224, 0.3) !important; box-shadow: 0 30px 60px rgba(0,0,0,0.5); }
      `}</style>
    </main>
  );
}
