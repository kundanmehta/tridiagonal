...
export default function AboutUsClient({ initialData = {} }) {
  ...
  const dHero = initialData.heroSection || {};
  const heroTitleHtml = dHero.title || 'Delivering Process <span class="gradient-text">Excellence</span>';
  const heroDesc = dHero.description || 'Technology catalyst to provide operations excellence with combination of skillsets and advanced technologies.';
  const heroBg = dHero.bgImage || '/images/about-hero-bg.png';

  const dIntro = initialData.introSection || {};
  const introParas = dIntro.paragraphs?.length > 0 ? dIntro.paragraphs : [
  ...
  const credsArray = dIntro.credentials?.length > 0 ? dIntro.credentials : [
  ...
  const dGlance = initialData.atAGlanceSection || {};
  const glanceHeadingHtml = dGlance.heading || 'Tridiagonal Solutions at a <span class="gradient-text">glance</span>';
  const glanceCards = dGlance.cards?.length > 0 ? dGlance.cards : atAGlanceData;

  const dLeadership = initialData.leadershipSection || {};
  const leaderHeading = dLeadership.heading || 'Our Leadership Team';
  const leaderDesc = dLeadership.description || 'Empowering Excellence, Inspiring Growth.';
  const leadersArray = dLeadership.members?.length > 0 ? dLeadership.members : leadershipTeam;

  const dCoreTeam = initialData.coreTeamSection || {};
  const coreHeading = dCoreTeam.heading || 'Core Team';
  const coreDesc = dCoreTeam.description || 'Delivering Value to the Customers';
  const coreArray = dCoreTeam.members?.length > 0 ? dCoreTeam.members : coreTeam;

  const dTimeline = initialData.timelineSection || {};
  const timelineHeading = dTimeline.heading || 'Tridiagonal Solutions Evolution';
  const timelineArray = dTimeline.events?.length > 0 ? dTimeline.events : [
  ...
  const dCta = initialData.ctaSection || {};
  const ctaHeadingHtml = dCta.heading || 'Looking for a <span class="gradient-text">Trusted Partner</span> for<br />executing your programs?';
  const ctaDescHtml = dCta.description || 'We bring together unparalleled expertise with combination of skillsets and technology to<br />address your digital, computational and testing needs';
  const ctaBtnText = dCta.buttonText || 'CONTACT US NOW';
  const ctaBtnLink = dCta.buttonLink || '/contact-us';
  const ctaBg = dCta.bgImage || '/hubfs/topography-bg.webp';
    ...
    <main style={{ paddingTop: 'var(--nav-height)' }}>
      ...
      <section
        ...
        style={{
          position: 'relative',
          overflow: 'hidden',
          background: `url("${heroBg}") center center / cover no-repeat, #242424`,
          minHeight: 'auto',
          padding: '80px 0 60px',
        }}
      >
      ...
      <section className="section-pad" style={{ background: '#1a1a1a' }}>
        <div className="content-wrapper-lg">
          <div style={{ marginBottom: '80px', width: '100%', maxWidth: '100%' }}>
            {introParas.map((pText, i) => (
              <p key={i} className="section-desc" style={{ color: '#fff', fontSize: '16px', lineHeight: 1.8, marginBottom: i === introParas.length - 1 ? '0' : '20px', maxWidth: '100%', width: '100%' }} dangerouslySetInnerHTML={{ __html: pText }} />
            ))}
          </div>
      ...
      <section className="section-pad" style={{ background: '#111', position: 'relative' }}>
        <div className="content-wrapper-lg">
          <h2 className="section-title" style={{ color: '#fff', fontSize: '40px', fontWeight: '700', textAlign: 'center', marginBottom: '80px' }} dangerouslySetInnerHTML={{ __html: glanceHeadingHtml }} />
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '80px' }}>
            {glanceCards.map((item, i) => (
      ...
      <section id="our-team" className="section-pad" style={{ background: '#1c1c1c' }}>
        <div className="content-wrapper-lg">
          <div style={{ textAlign: 'center', marginBottom: '60px' }}>
            ...
            <p className="section-desc" style={{ color: 'var(--color-teal)', margin: '0 auto' }}>{leaderDesc}</p>
          </div>
      ...
      <section className="section-pad" style={{ background: '#242424' }}>
        <div className="content-wrapper-lg">
          <div style={{ textAlign: 'center', marginBottom: '60px' }}>
            ...
            <p className="section-desc" style={{ color: 'var(--color-teal)', margin: '0 auto' }}>{coreDesc}</p>
          </div>

          <div className="leaders-grid-new">
            {coreArray.map((leader, i) => (
              <LeaderCard key={i} leader={leader} onClick={() => setSelectedLeader(leader)} />
            ))}
          </div>
        </div>
      ...
      {selectedLeader && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', backdropFilter: 'blur(5px)' }} onClick={() => setSelectedLeader(null)}>
          <div style={{ background: '#1a1a1a', borderRadius: '20px', padding: '40px', maxWidth: '800px', width: '100%', position: 'relative', maxHeight: '90vh', overflowY: 'auto', border: '1px solid rgba(255,255,255,0.1)' }} onClick={e => e.stopPropagation()}>
            <button suppressHydrationWarning={true} onClick={() => setSelectedLeader(null)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'none', border: 'none', color: '#fff', fontSize: '32px', cursor: 'pointer', zIndex: 10 }}>×</button>
      ...
      <section ref={timelineRef} style={{ padding: 'clamp(40px, 8vw, 60px) 0', background: '#111', position: 'relative', overflow: 'hidden' }}>
         <div className="content-wrapper-lg">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px', marginBottom: 'clamp(30px, 6vw, 60px)' }}>
              <h2 className="section-title" style={{ color: 'var(--color-teal)', fontSize: 'clamp(28px, 5vw, 40px)', fontWeight: '700', margin: 0 }}>{timelineHeading}</h2>
              <div style={{ display: 'flex', gap: '10px' }}>
                <button suppressHydrationWarning={true} onClick={() => scrollTimeline('left')} style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#242424', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.3s' }} onMouseEnter={e=>e.currentTarget.style.background='var(--color-teal)'} onMouseLeave={e=>e.currentTarget.style.background='#242424'}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6"/></svg>
                </button>
                <button suppressHydrationWarning={true} onClick={() => scrollTimeline('right')} style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#242424', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.3s' }} onMouseEnter={e=>e.currentTarget.style.background='var(--color-teal)'} onMouseLeave={e=>e.currentTarget.style.background='#242424'}>
      ...
      >
        ...
        >
          ...
          <div style={{ position: 'relative', zIndex: 1, maxWidth: '755px', margin: '0 auto' }}>
            <h2 className="section-title" style={{ color: '#fff', fontSize: '45px', fontWeight: '700', lineHeight: 1.25, marginBottom: '24px' }} dangerouslySetInnerHTML={{ __html: ctaHeadingHtml }} />
            <p className="section-desc" style={{ color: 'rgba(255,255,255,0.9)', fontSize: '18px', lineHeight: 1.7, marginBottom: '40px' }} dangerouslySetInnerHTML={{ __html: ctaDescHtml }} />
            <Link
              href={ctaBtnLink}
